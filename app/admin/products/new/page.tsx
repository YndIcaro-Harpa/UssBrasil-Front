...
// Interface para variação de cor com imagens
...
interface ProductFormData {
  ...
  hasVariations: boolean  // Se true, o estoque é calculado pela soma das variações
}
...
export default function NewProductPage() {
  ...
  // Estados para adicionar variações
  ...
  const [variationForm, setVariationForm] = useState({
    ...
    stock: 0,
    priceAdjustment: 0,
    serialNumbers: '' // Separado por vírgula
  })
  ...
  }, [formData.productVariations, formData.hasVariations])
  ...
  const calculatePrices = (data: Partial<ProductFormData>) => {
    ...
    // Preço base para cálculos (com desconto se houver, senão preço normal)
    const basePrice = discountPrice > 0 ? discountPrice : price

    // Calcular preço final Stripe (desconto adicional sobre o preço base)
    const stripeFinalPrice = stripeDiscount > 0 
      ? basePrice * (1 - stripeDiscount / 100) 
      : 0

    // Preço final é o menor preço disponível (Stripe se tiver, senão desconto, senão normal)
    ...
    // Calcular margem de lucro: ((preço final - custo) / preço final) * 100
  ...
  // Estados para especificações dinâmicas
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productData = {
        ...
        colors: JSON.stringify(formData.colorVariations),
      ...
      // Fazer chamada à API para criar o produto
      const response = await api.products.create(productData as any, token || undefined)
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Será usado na URL: /produto/{formData.slug || 'seu-slug'}
                </p>
              </div>
          ...
          >
            ...
            <div className="space-y-6">
              {/* Cores com Imagens */}
              <div>
                <FormLabel label="Variações de Cor (com imagens)" />
                <p className="text-sm text-gray-500 mb-3">Adicione cores e faça upload de imagens específicas para cada cor.</p>
                      ...
                      <div key={index} className="border border-gray-200 rounded-xl p-4 bg-gray-50">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <span className="font-medium text-gray-900">{colorVar.name}</span>
                            <span className="text-sm text-gray-500">({colorVar.images.length} imagens)</span>
                          </div>
                          <button
                            type="button"
                        ...
                        <div className="grid grid-cols-4 gap-2 mb-3">
                          {colorVar.images.map((img, imgIndex) => (
                            <div key={imgIndex} className="relative group">
                              <img
                                src={img}
                                alt={`${colorVar.name} ${imgIndex + 1}`}
                                className="w-full h-20 object-cover rounded-lg border border-gray-200"
                              />
                              <button
                                type="button"
                                onClick={() => {
                                  const newVariations = [...formData.colorVariations]
                                  newVariations[index].images = newVariations[index].images.filter((_, i) => i !== imgIndex)
                                  setFormData(prev => ({ ...prev, colorVariations: newVariations }))
                                }}
                          ...
                          <button
                            ...
                            className="w-full h-20 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-gray-400 hover:border-indigo-400 hover:text-indigo-500 transition-colors"
                          >
                ...
                <div className="flex gap-2 items-end">
                  <div className="flex-1">
                    <Input
                      ...
                      placeholder="Nome da cor (ex: Rosa, Azul Marinho...)"
                    />
                  </div>
                  <div>
                    <input
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (newColor.trim()) {
                        setFormData(prev => ({
                          ...prev,
                          colorVariations: [...prev.colorVariations, { 
                            name: newColor.trim(), 
                            hexCode: newColorHex,
                            images: [] 
                          }]
                ...
                {/* Input oculto para upload de imagens por cor */}
                <input
                    ...
                    try {
                      for (const file of Array.from(files)) {
                        ...
                        if (response.ok) {
                          ...
                          if (data.url) {
                            setFormData(prev => {
                              const newVariations = [...prev.colorVariations]
                              newVariations[selectedColorIndex].images.push(data.url)
              ...
              {/* Tamanhos */}
              <div>
                <FormLabel label="Tamanhos Disponíveis" />
                <div className="flex flex-wrap gap-2 mb-2">
                  {formData.sizes.map((size, index) => (
                    <span key={index} className="inline-flex items-center gap-1 px-3 py-1.5 bg-green-100 text-green-700 rounded-full text-sm">
                      {size}
                      <button
                        type="button"
                        onClick={() => setFormData(prev => ({
              ...
              {/* Armazenamento */}
              <div>
                <FormLabel label="Opções de Armazenamento" />
                <div className="flex flex-wrap gap-2 mb-2">
                  {formData.storage.map((option, index) => (
                    <span key={index} className="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-100 text-purple-700 rounded-full text-sm">
                      {option}
                      <button
                        type="button"
                        onClick={() => setFormData(prev => ({
